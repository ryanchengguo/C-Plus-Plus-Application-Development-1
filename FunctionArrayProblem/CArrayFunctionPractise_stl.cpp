...
template <size_t N>
int arr_max(const array<int, N> &a);

// Search array and return the index of the max value in the array
template <size_t N>
size_t arr_index_of_first_max(const array<int, N> &a);

// Search array and return the index of the last max value in the array
template <size_t N>
size_t arr_index_of_last_max(const array<int, N> &a);

// Search array and return index of first occurence of value in the array
// Return -1 if value is not in the array
template <size_t N>
size_t arr_index_of_value_first(const array<int, N> &a, int value);

// Search array and return index of last occurence of value in the array
// Return -1 if value is not in the array
template <size_t N>
size_t arr_index_of_value_last(const array<int, N> &a, int value);

// Compare 2 arrays and return true if they are equal and false otherwise
// You can assume that the 2 arrays are the same size
template <size_t N>
bool arr_compare(const array<int, N> &a, const array<int, N> &b);



int main()
{
	array<int, SIZE1> arr1 = { 10, 20, 75, 15, 35, 12, 65, 15, 75, 55};
	array<int, SIZE1> arr2 = { 10, 20, 75, 15, 35, 12, 65, 15, 75, 55 };
	array<int, SIZE1> arr3 = { 10, 20, 75, 15, 100, 12, 65, 15, 75, 55 };
...
//cstyle file int type go through, stl file int type build failed... <--------Question
...
// Compare 2 arrays and return true if they are equal (true) and false (false) otherwise
// You can assume that the 2 arrays are the same size
template <size_t N>
bool arr_compare(const array<int, N> &a, const array<int, N> &b)
{
//    SIZE1 used for arr1 & arr3
//    if(a.size() != b.size()){
//        return false;
//    }
